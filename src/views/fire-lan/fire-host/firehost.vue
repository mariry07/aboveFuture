              ...
              <Form ref="searchForm" :model="searchForm" inline :label-width="70" class="search-form">
                <span v-if="drop">
                  ...
                  <Form-item label="运行状态" prop="status">
                    <Select
                      v-model="searchForm.runningState"
                      placeholder="请选择"
                      clearable
                      style="width: 200px"
                    >
                      <Option v-for="item in deviceTypeList" :key="item.id" :value="item.id">{{item.name}}</Option>
                    </Select>
                  </Form-item>
    ...
    <!-- 编辑按钮对应的Form表单 -->
      <Form ref="hostConfigForm" :model="hostConfigForm" :label-width="90" :rules="hostConfigFormValidate">
        <Row>
          ...
          <Col span="11">
            <FormItem label="主机类型:" prop="deviceType">
              <Select v-model="hostConfigForm.deviceType" disabled  placeholder="无线烟感主机">
          ...
          <Col span="11">
            <FormItem label="版本号:" prop="version">
              <Select v-model="hostConfigForm.version" @on-change="showVersion"  placeholder="请选择">
                <Option v-for="d in deviceType" :value="d.id" :key="d.id">{{d.name}}</Option>
              </Select>
            </FormItem>
          </Col>
          <Col span="11">
        ...
        <Row>
          <h2>参数配置</h2>
          <h2>地图配置</h2>
          <Col span="22" style="height: 50px;">
            <FormItem label="地理环境选择:" prop="outdoor" label-width="100" style="margin-left:20px;">
              <RadioGroup v-model="door" @on-change="handledoor" size="large" style="height:50px;">
                <Radio :key="0" :label="0">室内</Radio>
                <Radio :key="1" :label="1">室外</Radio>
              </RadioGroup>
            </FormItem>
          </Col>
          <div v-show="indoorStatus">
            ...
            <FormItem ref="floorId" prop="floorId" label-width="5" style="width:300px; float:left">
        ...
        <div v-show="indoorStatus">
          <!--<div id="image-map" alt="alt" style="width:100%; height:400px;"></div>-->
        ...
        <!--<div style="width:100%; float:left;">-->
          <!--<div style="width:50%; float:left;">-->
            ...
            <!--</FormItem>-->
            <!--<FormItem label="版本号:" prop="version">-->
              <!--<Select v-model="hostConfigForm.version"  placeholder="请选择">-->
                <!--<Option v-for="d in deviceType" :value="d.code" :key="d.code">{{d.name}}</Option>-->
              <!--</Select>-->
            <!--</FormItem>-->
            <!--<FormItem label="SIM卡ID:" prop="sim">-->
              <!--<Input  v-model="hostConfigForm.sim" autocomplete="off"/>-->
          ...
          <!--<div style="width:50%; float:left;">-->
            ...
            <!--<FormItem label="主机类型:" prop="deviceType">-->
              <!--<Select v-model="hostConfigForm.deviceType" disabled  placeholder="无线烟感主机">-->
                <!--<Option :value="1001">无线烟感主机</Option>-->
                <!--&lt;!&ndash; <Option :value="1002">消防用水主机</Option> &ndash;&gt;-->
              <!--</Select>-->
            <!--</FormItem>-->
            <!--<FormItem label="主机编号:" prop="code">-->
              <!--<Input  autocomplete="off"  v-model="hostConfigForm.code"/>-->
        ...
        <!--<div v-if="indoorStatus"  style="width:100%; height:400px;">-->
          <!--&lt;!&ndash;<img src="../../../assets/bg.png" alt="alt" style="width:100%; height:400px;">&ndash;&gt;-->
          <!--<img :src=imagePath alt="alt" style="width:100%; height:400px;">-->
        <!--</div>-->
...
export default {
  ...
  data() {
    ...
    return {
      ...
      hostConfigFormValidate: {
        ...
        version:[
          {required:true ,type:'number',msesage:'版本号不能为空',trigger:'blur'}
        ],
        parentTitleId: [
      ...
      columns: [
...

        {
          ...
          align: "center",
          render: (h, params) => {
            if(params.row.cameras!= null && params.row.cameras.length===1){
              return h('div',[
                      h(
                              "Tag",
                              {
                              ...
                              params.row.cameras[0].name
                      )
              ])
            }else if(params.row.cameras!= null &&params.row.cameras.length >1){
              return h('div',[
                      h(
                              "Tag",
                              {
          ...
          render:(h,params) =>{
            ...
            return h('i-switch',{
              props:{
                value:true,
                disabled:true,
                //value: params.row.commodityStatus //通过value控制Switch开关
              },
              scopedSlots:{
                open:()=>h('span',''),
                close:()=>h('span','')
  ...
  methods: {
    ...
    getdeviceVersion(){
      //this.loading=true;
      getdeviceVersion({deviceType:'7'}).then(res=>{
        if(res.status === 1){
    ...
    getDataList() {
      ...
      this.loading = true;
       getdeviceList(this.searchForm).then(res => {
        this.loading = false;
        if (res.status === 1) {
          this.data = res.dataValue.deviceList;
    ...
    delAll() {
                ...
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        ...
                        deleteForm.ids=ids
                        deleteDevice(deleteForm).then(res => {
                            this.operationLoading = false;
                            if (res.status === 1) {
                                this.$Message.success("操作成功");
                                this.clearSelectAll();
    ...
    details(v) {
      let query = { type: 7, id: v.id, name: v.name, code: v.code, version: v.version, addTime: v.addTime, position: v.position, updateTime: v.updateTime, companyId: v.companyId, chargeName:v.chargeName, chargePhone: v.chargePhone, backRoute: this.$route.name };
      this.$router.push({
        name: "details_firehost",
        query: query
      });
    },
      ...
      this.$refs.hostConfigForm.validate(valid => {
        if (valid) {
          if (this.modalType === 0) {
            ...
            delete this.hostConfigForm.cameras;
  ...
  //禁用设备方法  wrz
    ...
    //   let _this=this;
    //   if(e.runningState !='2'){
    //       _this.$Message.warning("只能禁用状态【启用】的设备");
    //       statustrue = false;
    //     }
    //   this.$Modal.confirm({
    //     title:"确定禁用",
    ...
    //       changeDeviceStatus(this.statusForm).then(res =>{
    //         if (res.status === 1) {
    ...
    batchEnable(){
      ...
      let statustrue=true
       if (this.selectCount <= 0) {
        this.$Message.warning("您还未选择要启用的设备");
        return;
      }
       this.selectList.forEach(function(e) {
        if(e.runningState !='5'){
          _this.$Message.warning("只能启动状态【禁用】的设备");
          statustrue = false;
        }
      });
    ...
    batchDisabling() {
      ...
      if (this.selectCount != 1) {
        this.$Message.warning("您只能选择一条数据进行操作");
        return;
      }
      this.selectList.forEach(function(e) {
        if(e.runningState =='5'){
      ...
      if(statustrue){
        this.$Modal.confirm({
          ...
          onOk: () => {
            //批量禁用，目前后端接口只接受单个id； 怎么通过接口改变runningStatus的值
            // let ids = "";
            // this.selectList.forEach(function(e) {
            //   ids += e.id + ",";
            // });
    ...
    //init leafletMap
    initLeafmap(){
      var map=L.map('image-map').setView([37.75,-122.23],17)
