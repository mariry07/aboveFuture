...
<template>
  <div class="search">
          ...
          <Row>
            <Form ref="searchForm" :model="searchForm" inline :label-width="70" class="search-form">
              ...
              <Form-item label="计划类型" prop="type">
                <Select v-model="searchForm.type" placeholder="请选择或输入搜索" filterable clearable>
    ...
    <Modal :title="modalTitle" v-model="modalVisible" :mask-closable='false' :width="860" :styles="{top: '30px'}">
      <Form ref="form" :model="form" :label-width="75" :rules="formValidate" label-position="left">
          <Row>
              <Col span="11">
              ...
              <Col span="11" offset="2">
                  <FormItem label="执行人" prop="doUserId">
          ...
          <Row>
              <Col span="11">
                  <Table height="280" border :columns="patrolMarkColumns" :data="patrolMarkData" ref="patrolMarkTable"></Table>
                  <div style="text-align: center;margin-top: 15px;">
                      <span>共&nbsp;{{patrolMarkTotal}}&nbsp;条</span>
                  </div>
              </Col>
              <Col span="2">
              ...
              <Col span="11">
                  <Table height="280" border :columns="selectedPatrolMarkColumns" :data="selectedPatrolMarkData" ref="selectedPatrolMarkTable"></Table>
                  <div style="text-align: center;margin-top: 15px;">
                      <span>共&nbsp;{{selectedPatrolMarkTotal}}&nbsp;条</span>
                  </div>
              </Col>
          </Row>
...
import {getPlanData, addPatrolPlan, deletePlan, getPatrolPlan, getDictDataByType, getAllUserData, getAllPatrolMark} from "@/api/index";
export default {
  name: "plan",
  components: {
    circleLoading
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            let re = "";
            if (params.row.isDoing == 1) {
              re = "进行中";
            } else if (params.row.isDoing == 0) {
               re = "已停止";
            }
            return h("div", re);
          }
      ...
      patrolMarkTotal: 0,
      /** 已选择巡更点列表**/
      selectedPatrolMarkTable: {
            pageNumber: 1, // 当前页数
            pageSize: 5, // 页面大小
      },
      selectedPatrolMarkTotal: 0,
      selectedPatrolMarkColumns: [
            {
                title: "名称",
                key: "name",
                align: "center",
  ...
  methods: {
    ...
    getDataList() {
      ...
      //获取全部巡更点
    getPatrolMarkDataList(){
        getAllPatrolMark().then(res => {
          if (res.success == true) {
              this.patrolMarkData = res.result;
              this.patrolMarkTotal = res.result.length;
          }
      });
      this.patrolMarkTotal = this.patrolMarkData.length;
    },
      //获取计划详情
    ...
    add() {
      this.getAllUser();
      this.getPatrolMarkDataList();
      this.modalTitle = "添加巡更计划";
      ...
      this.selectedPatrolMarkData = [];
      this.selectedPatrolMarkTotal = 0;
    },
    handelSubmit() {
      this.$refs.form.validate(valid => {
          if (valid) {
              ...
              delete this.form.id;
              this.selectedPatrolMarkData.forEach((item) => {
                  this.form.list.push(item.id);
              });
              //验证选择巡更人员
              if(this.form.doUserId == "" ){
                  this.$Message.warning("请选择巡更人员");
                  return;
              }
              //验证执行周期
              if(this.form.type == "" ){
    ...
    addToSelected(e){
        //e是左侧列表的元素
        this.selectedPatrolMarkData.push(e);
        this.selectedPatrolMarkTotal++;
    },
    moveSelected(e){
        //备选列表恢复
        this.selectedPatrolMarkData.forEach((item) => {
        ...
        //移除元素
        this.selectedPatrolMarkData.splice(e._index,1);
        this.selectedPatrolMarkTotal--;
    },
    detail(id){
        this.getPatrolPlanDetail(id);
        this.detailModalTitle = "查看巡更计划";
