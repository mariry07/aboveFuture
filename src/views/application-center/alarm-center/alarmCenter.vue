  ...
  <div class="search">
        <Row class="operation">
          <div style="width:90%; float:left">
            ...
            <Button @click="findall" icon="ios-search">查找</Button>
            <circleLoading v-if="operationLoading"/>
          </div>
            <div style="width:5%; float:left">
              <Button type="success" @click="statusSuccess">已处理{{processed}}</Button>
            </div>
            <div style="width:5%; float:left">
              <Button type="error" @click="statusError">未处理{{untreated}}</Button>
            </div>
          </Row>
          <Row>
              <Form ref="searchForm" :model="searchForm" inline :label-width="90" class="search-form">
                ...
                <Form-item label="报警类型" prop="alarmTypeName">
                  ...
                  >
                    ...
                    <Option :value='"水压告警"'>水压告警</Option>
                    <Option :value='"液位告警"'>液位告警</Option>
                    <Option :value='"温度告警"'>温度告警</Option>
                    <Option :value='"电量过低告警"'>电量过低告警</Option>
                  </Select>
                </Form-item>
          ...
          <!-- 处理分页 根据后台获取到的数据 通过点击事件进行页面的切换并展示对应页面上的数据 -->
...
export default {
  ...
  data() {
    ...
    //下面定义的是页面上绑定的数据， 注意名称要对应上，从后端取出来的数据也会放在这里，根据VUE动态绑定的属性，实现后端数据变化，页面上的数据也跟着变化的效果。
      ...
      //对应table表格的内容，注意key名称的定义，后台定义存放数据的Vo entity保持一致。 method区域中的方法中也会用到。
      columns: [
        {
          type: "selection",
          width: 60,
  ...
  methods: {
    ...
    // getAlarmTableData(){
    //   this.loading = true;
    //   this.getRequest("/alarmTable/getByCondition",this.searchForm).then(res =>{
    //     console.log(res);
    //     this.loading = false;
    //     if(res.success == true){
    //        this.data = res.result.content;
    ...
    //处理状态方法，对searchForm.status进行赋值，重新加载列表，实现检索条件的目的
    statusSuccess(){
